...
公开仓库默认将 Chroma 持久化目录写到 `.env` 中的 `./data/chroma`。若 Windows 中文路径环境下再次出现 HNSW 文件缺失或 collection 不可查询问题，建议仅在本地 `.env` 覆盖为 ASCII 目录。
...
状态（2026-06-05）：本机真实 bge-m3 + Chroma 索引已验证通过。collection `case_chunks_bge_m3_v1` 位于本地 Chroma 持久化目录，count=9621，元数据为 `embedding_provider=ollama`、`model_name=bge-m3`、`vector_dimension=1024`、`distance_metric=cosine`。`--resume --smoke --query-smoke` 实测 `embedding_success_rate=100.0`，近邻自检 `self_recall=true`，5 条手工 query 均返回候选。

## 8. 评测集与兜底映射（4.5）

评测数据建议放在 `data/external/LeCaRDv2-main`，也可通过 `--lecard-root` 指向任意本地目录。[LeCaRDv2 官方 README](https://github.com/THUIR/LeCaRDv2) 说明该数据集包含 800 条 query、55,192 份 candidate case，并提供 TREC qrels 相关性标注；如本地 `candidate/` 下已放入 `candidate.tar` 正文包，脚本可直接读取 tar，无需先解压。

```bash
# 1) 标准化 LeCaRDv2 test split 的 query/qrels，并生成术语映射
python apps/api/app/eval/prepare_lecardv2_eval.py \
  --lecard-root data/external/LeCaRDv2-main \
...
python apps/api/app/eval/bm25_baseline.py \
  ...
  --corpus data/external/LeCaRDv2-main/candidate \
